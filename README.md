# 深蓝 DeepBlue

一款运行于 Windows 的本地语音播报软件。打开软件即自动获取日期与日程数据，一键语音播报你今天的安排与即将截止的重要事项。核心功能完全本地运行；天气播报为可选联网功能，不开启则无任何网络请求，也不采集任何数据。

## 功能特性

- **书籍形态界面**：主界面以一本「晨光穿林」的书籍呈现——启动时是合上的封面（大篆「深蓝」金字），点击「翻开新的一页」后展开为对开双页：左页是今日信息，右页是播报文稿，正在播报的句子以暖金高亮
- **启动即就绪**：打开软件后 2 秒内自动完成日期与日程数据加载，无需手动刷新
- **语音播报**：逐句高亮显示当前播报进度，支持暂停 / 继续 / 停止；默认使用 Windows 系统语音，安装[语音增强包](#语音增强包可选)后可使用接近真人的「晓晓」自然语音
- **天气播报（可选）**：开启后自动播报今日天气、最高最低气温、降水概率，支持全球城市搜索，断网自动跳过不影响其他播报；数据源可切换 [Open-Meteo](https://open-meteo.com/)（默认）或[和风天气](https://www.qweather.com/)
- **今日安排**：播报你今天录入的全部事项，按时间升序朗读，重要事项（P0）会加上「重要」前缀
- **截止提醒**：播报未来 N 天内（可设置 1–30 天）优先级为 P0 / P1 的截止事项，自动忽略 P2 及以下
- **重复日程**：支持每天 / 每周 / 每月三种重复规则，无需反复录入
- **开机自启动（可选）**：在「设置 → 通用」勾选后，登录 Windows 时自动启动深蓝；仅写入当前用户的注册表（无需管理员权限），卸载软件时自动清除
- **个性化设置**：语音选择、语速（0.5x–2.0x）、提醒窗口天数、播报段落开关均可自定义
- **数据本地存储**：所有数据保存在 `%APPDATA%\DeepBlue` 目录下的 JSON 文件中，可随时备份或迁移
- **跨天自动刷新**：跨越零点继续使用时，自动检测数据过期并重新获取

## 系统要求

- Windows 10 / Windows 11（64 位或 32 位）
- .NET Framework 4.8+（Windows 10 1903 及以上版本系统自带，无需额外安装）
- 至少一个系统语音（中文系统默认自带；推荐在「设置 → 时间和语言 → 语音」中安装中文语音包以获得更好效果）

## 下载与安装

1. 前往本仓库的 [Releases](../../releases) 页面
2. 下载最新版本的 `DeepBlue-Setup-x.x.x.exe`
3. 双击运行安装向导，按提示完成安装（可选择是否创建桌面快捷方式）
4. 安装完成后从开始菜单或桌面启动「深蓝 DeepBlue」

卸载：通过「设置 → 应用 → 已安装的应用」或控制面板中的「深蓝 DeepBlue」条目卸载。卸载不会删除 `%APPDATA%\DeepBlue` 中的个人数据，如需彻底清理请手动删除该目录。

## 语音增强包（可选）

系统自带语音（如惠惠）较为机械。语音增强包为深蓝接入微软「晓晓」自然语音——神经网络合成，接近真人，**本地离线运行，无需联网**。

**安装**：

1. 在 [Releases](../../releases) 页面下载 `DeepBlue-Voice-x.x.x.exe`（约 56 MB）
2. 双击运行安装向导，完成即生效（与主程序先后顺序无关）
3. 启动深蓝，语音将自动默认为「晓晓」，也可在「设置 → 语音」中切换

**说明**：

- 增强包与主程序相互独立：卸载增强包后深蓝自动回退系统语音，不影响日程数据
- 增强包基于开源项目 [NaturalVoiceSAPIAdapter](https://github.com/gexgd0419/NaturalVoiceSAPIAdapter)（MIT 协议），将 Windows 讲述人的自然语音桥接为标准 SAPI5 引擎，其他支持 SAPI5 的软件同样可以使用
- 晓晓语音数据版权归属微软，仅供个人使用，请勿二次分发（详见安装目录内 `THIRD-PARTY-NOTICES.txt`）
- 安装目录 `C:\Program Files\DeepBlueVoice` 请勿移动或重命名
- 属非官方方案，极少数情况下可能随 Windows 更新失效，届时重新安装新版增强包即可

**从源码构建语音包**：运行 `installer\prep_voice.ps1`（下载适配器与语音包并组装载荷），再用 Inno Setup 编译 `installer\voice.iss`。

## 天气播报（可选）

在「设置 → 播报段落开关」中勾选「天气」，并在「天气」卡片中选择数据源、搜索选择城市，保存后即生效。播报形式如：「北京市今天晴，最高29度，最低15度。」

**数据源二选一**：

| 数据源 | 特点 | 适合 |
| --- | --- | --- |
| Open-Meteo（默认） | 免费、无需注册、无需 Key | 开箱即用 |
| 和风天气 QWeather | 国内气象数据质量高，含预警/生活指数 | 愿意注册开发者账号的用户（每月免费额度充足） |

选择和风天气需在「设置」中填写 API Host 与 API Key（在[和风天气开发者控制台](https://console.qweather.com/) → 设置中查看，2024 年后注册的账号使用专属 API Host），城市搜索将使用和风自己的地理接口（LocationID）。

**数据与隐私**：

- 天气功能默认关闭，未开启或未设置城市时深蓝不会发起任何网络请求
- 请求仅包含所选城市的位置信息（经纬度或 LocationID），不含任何个人身份数据
- 每日首次启动获取一次并缓存于 `%APPDATA%\DeepBlue\weather.json`，当天内重复启动直接读缓存不再联网；跨天自动重新获取
- 断网或接口超时（6 秒）时静默跳过天气段，不影响日期、安排、截止提醒的正常播报
- 命令行运行 `DeepBlue.exe --weathertest` 可诊断网络与接口连通性（按当前设置的数据源测试）

## 使用方法

1. 启动软件，日期与日程数据自动加载（左下角显示就绪状态）
2. 点击封面右上角的「···」菜单录入你的安排：
   - **日程**：带具体日期（或重复规则）与时间的事项，如「周三 9:30 团队周会」
   - **截止**：带截止日期的事项，如「9 月 10 日前提交季度报告」，可设置优先级 P0–P3
   - 每条事项支持添加备注，备注内容也会被播报
3. 点击封面上的「翻开新的一页」，书籍展开为对开双页，深蓝将语音播报：问候语 + 日期星期 → 今日安排 → 截止提醒 → 结束语
4. 播报过程中可随时暂停、继续或停止

## 从源码构建

本项目不依赖任何第三方库或 SDK，仅使用 Windows 内置的 .NET Framework C# 编译器（语音合成使用系统自带的 System.Speech 组件）：

```cmd
git clone https://github.com/beginner-study/shenlan.git
cd shenlan
build.cmd
```

编译产物输出至 `build\DeepBlue.exe`，可直接运行。

生成标准安装包（可选）：安装 [Inno Setup 6](https://jrsoftware.org/isinfo.php) 后执行：

```cmd
"C:\Program Files (x86)\Inno Setup 6\ISCC.exe" installer\shenlan.iss
```

安装包输出至 `build\DeepBlue-Setup-x.x.x.exe`（约 4 MB）。

## 运行自测

源码内置 85 项逻辑自测（覆盖问候语、时间口语化、重复规则、优先级过滤、播报稿生成、默认语音选择、双数据源天气代码与描述、缓存匹配、存储往返、开机自启动注册表读写等），构建脚本完成后会自动运行：

```cmd
build.cmd
type build\selftest.txt
```

## 项目结构

```
shenlan/
├── src/                  # C# 源码（纯 WinForms，.NET Framework 4.8）
│   ├── Program.cs        # 入口：单实例检测
│   ├── MainForm.cs       # 主窗体：封面态 → 展开态状态机
│   ├── CoverPanel.cs     # 封面态：合上的书（内嵌 ··· / × / 翻开新的一页）
│   ├── BookPanel.cs      # 展开态：对开双页（左页今日信息 + 右页播报文稿）
│   ├── LeftPage.cs       # 左页：日期、天气、今日安排、截止提醒
│   ├── GlassMenu.cs      # 毛玻璃下拉菜单
│   ├── Assets.cs         # 封面合成：插画 + 大篆金字 + 水印处理
│   ├── ScheduleForm.cs   # 日程管理：增删改查、重复规则、优先级
│   ├── SettingsForm.cs   # 设置：语音、语速、提醒窗口、段落开关、天气、开机自启动
│   ├── AutoStart.cs      # 开机自启动：HKCU Run 注册表键读写
│   ├── ScriptEngine.cs   # 播报稿引擎：日期/天气/今日/截止四段式脚本生成
│   ├── BroadcastEngine.cs# TTS 播报引擎：逐句播报、暂停/继续/停止、默认语音选择
│   ├── WeatherEngine.cs  # 天气引擎：Open-Meteo / 和风双源、每日缓存
│   ├── WeatherTest.cs    # 天气网络诊断（--weathertest）
│   ├── Models.cs         # 数据模型与 JSON 本地存储
│   ├── SelfTest.cs       # 内置自测框架
│   └── Ui.cs             # UI 样式辅助：森林绿金色板、大篆字体加载
├── assets/
│   ├── app.ico                        # 应用图标
│   ├── cover-forest.jpg               # 封面插画（晨光穿林）
│   ├── JFZSKSealScript-V2.5.ttf       # 大篆字体（随软件分发，无需安装）
│   └── JFZSKSealScript-OFL-LICENSE.txt
├── installer/
│   ├── shenlan.iss       # 主程序安装脚本
│   ├── voice.iss         # 语音增强包安装脚本
│   ├── prep_voice.ps1    # 语音增强包载荷准备脚本
│   └── THIRD-PARTY-NOTICES.txt
└── build.cmd             # 一键编译脚本
```

## 路线图

- ~~更自然的语音~~：V1.1 已提供可选语音增强包（晓晓自然语音，本地离线）
- ~~天气数据模块~~：V1.2 已上线（Open-Meteo 免费源，可选开启，默认关闭）
- ~~界面美化~~：V1.4 全新书籍形态界面（封面 → 对开双页）
- ~~开机自启动选项~~：已完成（设置 → 通用，用户级注册表，卸载自动清理）
- 更多播报场景（如晚安回顾）

## 备注

- 当前软件的日程逻辑仍有bug：设置个从昨天开始后天截止的日程，到今天时，软件就会认为这个日程已过期，给划掉了，不予播报，除非是P1紧急度及以上的才会播报

## 许可证

[MIT License](./LICENSE)

封面大篆字体「敬峰中山王篆」（JFZSKSealScript）以 [SIL OFL 1.1](./assets/JFZSKSealScript-OFL-LICENSE.txt) 协议随软件分发，版权归[刘敬峰](https://github.com/jeffi369/JFZSKSealScript)所有。
