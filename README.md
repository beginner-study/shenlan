...
- **语音播报**：使用 Windows 系统自带语音（TTS），逐句高亮显示当前播报进度，支持暂停 / 继续 / 停止
...
安装包输出至 `build\DeepBlue-Setup-1.0.0.exe`（约 1 MB）。

## 运行自测

源码内置 33 项逻辑自测（覆盖问候语、时间口语化、重复规则、优先级过滤、播报稿生成、存储读写等），构建脚本完成后会自动运行：
...
│   ├── BroadcastEngine.cs# TTS 播报引擎：逐句播报、暂停/继续/停止
│   ├── Models.cs         # 数据模型与 JSON 本地存储
│   ├── SelfTest.cs       # 内置自测框架
│   └── Ui.cs             # UI 样式辅助
├── assets/app.ico        # 应用图标
├── installer/shenlan.iss # Inno Setup 安装脚本
└── build.cmd             # 一键编译脚本
```

## 路线图

- **V1.1+**：天气数据模块（独立可选模块，接入天气数据源后播报天气段；V1.0 中设置页的天气项已预留并置灰）
- 开机自启动选项
- 更多播报场景（如晚安回顾）
