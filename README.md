...
- **天气播报（可选）**：开启后自动播报今日天气、最高最低气温、降水概率，支持全球城市搜索，数据来自 [Open-Meteo](https://open-meteo.com/)，断网自动跳过不影响其他播报
...
在「设置 → 播报段落开关」中勾选「天气」，并在「天气城市」卡片中搜索选择城市（支持中文，全球城市均可），保存后即生效。播报形式如：「北京市今天晴，最高29度，最低15度。」

**数据与隐私**：

- 数据源为 [Open-Meteo](https://open-meteo.com/)，免费、无需注册、无需 API Key（非商业用途，数据许可 CC BY 4.0）
- 天气功能默认关闭，未开启或未设置城市时深蓝不会发起任何网络请求
- 请求仅包含所选城市的经纬度，不含任何个人身份数据
- 每日首次启动获取一次并缓存于 `%APPDATA%\DeepBlue\weather.json`，当天内重复启动直接读缓存不再联网；跨天自动重新获取
- 断网或接口超时（6 秒）时静默跳过天气段，不影响日期、安排、截止提醒的正常播报
- 命令行运行 `DeepBlue.exe --weathertest` 可诊断网络与接口连通性
...
源码内置 57 项逻辑自测（覆盖问候语、时间口语化、重复规则、优先级过滤、播报稿生成、默认语音选择、天气代码与描述、缓存读写、存储往返等），构建脚本完成后会自动运行：
